'''
Current clamp demo using Allen Brain's model 472434498.
...
from neuron472434498 import Neuron472434498
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472434498(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472434498; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-86.9853973389)
        h.fcurrent()
